...
두번째 파트는 일반적으로 랜덤하게 생성한다. 랜덤하게 만들 것. 주의점은 다른 계좌주와 계좌번호가 겹쳐서는 안된다.
...
[잔금 조회기능]
출금 비밀번호를 체크 후 잔금을 조회하도록 한다. 마찬가지로 비밀번호가 3회 틀리면 출금기능을 막음과 동시에 잔금조회도 불가능하게 해야 한다.

캡슐화, 은닉화가 핵심이므로 모든 돈의 이동 과정은 메인 클래스에서 알 수 없도록 해야한다.

=========================================<2차 도전과제>=========================================
A계좌에서 B계좌로 송금 기능을 구현한다. 다른 은행에 따른 수수료는 고려하지 않는다.
단, 송금기능을 메인 클래스로 구현하지 말 것. 캡슐화, 은닉화가 핵심이므로 모든 돈의 이동 과정은 메인 클래스에서 알 수 없도록 해야한다.
